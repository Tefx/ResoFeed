...
printf "[CHECK] host=%s stack=%s\n" "$(hostname -s)" "$(basename "$PWD")"
...
Stop when the host is not `tefx-mbp-personal`, the directory is not `resofeed-caddy`, Docker/Compose is unavailable, the running stack uses an alternate image repository, `/data` is not the expected named volume, secrets are unavailable for the existing runtime boundary, or Tailnet TCP/443 points elsewhere.
