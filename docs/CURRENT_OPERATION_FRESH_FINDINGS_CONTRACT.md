...
Compatibility resolution note: `docs/ARCHITECTURE.md:949-965` has been reconciled to the same display/runtime vocabulary (`background_ingest`, `manual_ingest`, `source_fetch`, `library_reprocess`) and `actor_kind` field locked here. Internal guards may still map from low-level implementation inputs, but HTTP/MCP/frontend contract surfaces must not expose the older `kind`/`scope` pair as canonical current-operation shape.
