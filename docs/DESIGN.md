...
- Direct Inspector URLs encode every opaque item ID as `~` plus unpadded RFC4648 base64url. UI navigation and API requests use the same canonical token; malformed or non-canonical tokens do not become item IDs.
- Search URLs use `q` plus the documented optional lexical filters. Valid cold loads, refreshes, Back, and Forward restore fields and re-run lexical retrieval from the URL. Validation occurs before requests or history writes, and history state contains only bounded route primitives—never results, items, saved searches, or reading history.
