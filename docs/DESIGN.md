...
- Inspector uses full-screen navigation with back behavior and preserved feed scroll. Mobile Inspector/detail view has one sticky top back row that remains visible while reading and replaces the global `RESOFEED` banner on that route. Its visible text and accessible name must describe the deterministic destination: `Return to TODAY` / `返回今日` for a recorded Feed/TODAY origin, `Return to Search` / `返回搜索` for a recorded Search origin, and `Return to Feed` / `返回列表` for the external/cold-entry fallback to `/`. The title uses `{typography.inspector-title}`, body uses `{typography.payload}`, with 20–24px horizontal padding.
...
Keyboard and accessibility: token input receives initial focus, submit is reachable by keyboard, and rejection uses `aria-live="assertive"`. On non-Inspector routes, accepted state moves focus to the Steer input or first feed item. On an Inspector item route, the item-route recovery contract governs instead: accepted state moves focus to the Inspector heading or current route error state while preserving the route, surface, and title.
...
#### Item Deep-Link Inspector Route
The complete accepted contract is `docs/ITEM_DEEP_LINKS.md`. This subsection supersedes earlier RF-BUG clauses only where they require browser and API routes to share `~base64url`. The deep-link surface reuses the existing Item Inspector; no second reader, modal detail implementation, or public anonymous summary is permitted.

Canonical browser links use `/items/{percent-encoded raw item_id}` for the exact non-empty Unicode-scalar domain defined in the deep-link contract, excluding U+0000–U+001F and U+007F–U+009F. The valid opaque IDs `.` and `..` use the reserved canonical segments `!.` and `!..`; ordinary raw IDs beginning with `!` encode that byte as `%21`. Valid legacy `~base64url` browser routes remain compatible and canonicalize with `replaceState`. Item HTTP operations independently use `/api/items/~{unpadded RFC4648 base64url(item_id)}`; the UI must not reuse one token grammar for both surfaces. Feed and Search item activation updates the address bar through the shared item-link helper. On desktop, the existing Feed/Search plus Inspector composition remains visible; on narrow surfaces the Inspector remains a full-screen route.

Cold load, refresh, authentication retry, Retry, Back, Forward, and automatic first-item selection are read-only navigation. They do not mark inspection, report delivery, or change resonance. A deliberate item activation may retain the existing single human-inspection marker, but route synchronization must not repeat it. The marker waits for the item read outcome when duplicate resolution is possible and targets only the returned `item.id`.

For dispatchable item routes, the Inspector landmark remains mounted through loading and error states. Loading uses a polite status announcement. Async failures use an alert. Dispatchable invalid routes, not-found items, temporary API/network failures, summary-unavailable details, resolved duplicates, and broken duplicate targets remain visibly distinct. Invalid or missing items must never cause an unrelated first Feed item to appear beneath the item URL. Every SPA error state includes the same route-aware return action as the sticky Close row; temporary failures also include Retry. Its visible text and accessible name are `Return to TODAY` / `返回今日` for a recorded Feed/TODAY origin, `Return to Search` / `返回搜索` for a recorded Search origin, and `Return to Feed` / `返回列表` for an external/cold entry. The action invokes the visible-Close semantics in `docs/ITEM_DEEP_LINKS.md` §7.4: it restores the named recorded origin against current data, or replaces an external/cold item entry with `/` and establishes the fresh Feed fallback without pushing a Feed entry. This binding applies to dispatchable-invalid, not-found, temporary API/network, and every other SPA error exposing the return action. A raw malformed-percent cold load is the documented server-level HTTP `400` case and does not load this Inspector surface.

Owner Token Prompt appears in place without changing the deep-link URL. Before token hydration, every canonical, legacy, or dispatchable invalid item route must already expose the Inspector-owned first surface and exact document title `RESOFEED · INSPECTOR`; TODAY or Search must not appear first. Failed authentication preserves the target, surface, and title and permits retry. After accepted authentication, re-read and discriminate the current route: canonical and legacy item routes load detail, while a dispatchable invalid route retains the invalid URL and localized invalid-link Inspector error and issues zero item API requests. Successful authentication returns focus to the Inspector heading or current route error state without changing that identity. This item-route-specific focus rule governs over the generic Owner Token Prompt target, which applies only to non-Inspector routes.

Visible Close and browser Back/Forward restore the recorded Feed/Search URL and filters against current data. Search restoration re-runs lexical retrieval against the current corpus and never persists result rows or arrays in history. After current Search results and layout are ready, restore the recorded selected indication and originating-row focus only when both referenced items remain; if either disappeared, restore no selection and focus the Search query control. Clamp the stored Search scroll coordinate to the current result-region bounds for the active scroll owner. Recorded Feed restoration also waits for current Feed data and layout. Restore the recorded selection and originating-row focus only when both items remain. If either is absent and Feed is non-empty, desktop selects the first current Feed item, narrow clears selection, and both layouts focus the first current Feed item; if Feed is empty, both clear selection and focus the Steer input. Clamp recorded Feed-pane and window scroll coordinates to current bounds and apply the active owner's clamped coordinate without resetting a valid coordinate solely because an item disappeared. Close uses browser Back when the item came from Feed/Search. For an external direct entry, visible Close replaces the current item history entry with `/` and establishes a fresh Feed fallback instead of restoring any background Feed state. After Feed data and layout are ready, reset both the Feed-pane and window scroll coordinates to `0`. If Feed has items, desktop selects the first item under the normal TODAY auto-selection rule, while narrow keeps no item selected until deliberate activation; both layouts focus the first Feed item. If Feed is empty, both layouts clear selection and focus the Steer input. A subsequent Back cannot reopen the closed Inspector. Native mobile edge-swipe remains ordinary browser/platform Back: it restores a recorded Feed/Search origin or follows pre-existing external history. Narrow-route `Escape` remains the distinct keyboard shortcut defined below: after inner controls decline the key, it returns to TODAY and does not claim visible-Close or browser-Back parity.

When a duplicate link resolves to an authority, the Inspector displays the authoritative content and one low-chrome merge-relation line. A broken target preserves the requested item's readable detail and shows one unavailable-target relation line. The requested URL remains stable. Copy-link behavior uses the shared `itemAppUrl` helper and prefers the authoritative item ID after successful resolution.

Every explicit item-scoped Inspector action uses the returned `ItemReadResult.item.id`: the requested ID for ordinary and broken-target reads, and the authoritative target ID for resolved duplicates. This applies to the deliberate inspection marker, Resonate, and Re-ingest. The UI must not also mutate the requested duplicate after resolution or the missing target of a broken relation.
...
A recorded Feed return from an item route resolves selection and focus against the current Feed after data and layout are ready. Restore the recorded selection and originating-row focus only when both items remain. If either is absent and Feed is non-empty, desktop selects the first current item, narrow has no selection, and both layouts focus the first current item. If Feed is empty, both layouts have no selection and focus the Steer input. Clamp recorded Feed-pane and window scroll coordinates to current bounds and apply the active owner's clamped coordinate; disappearance of a referenced item alone does not reset a valid stored coordinate to `0`.
...
Selection is ephemeral browser state. Preserve the current selected ID and readable preview across desktop/narrow viewport changes, direct-route pending/failure/success, Back, focus return, and Escape only while that ID remains valid in the current surface data; recorded Feed returns use the deterministic missing-reference outcomes above. Do not persist selection or create reading history, a modal detail system, or a second detail route model.
...
- **useFor**: one visible direct command `[REGENERATE]` / `[重新生成]`, an `Options` / `选项` disclosure for temporary OpenRouter model selection loaded from canonical `GET /api/runtime/openrouter-models` (with `GET /api/runtime/openrouter/models` compatibility-only) and optional extra prompt text, and result/conflict/error text for `POST /api/items/~{unpadded RFC4648 base64url(item_id)}/reingest` or matching MCP `reingest_item`.
- **targeting**: `item_id` is the current read result's returned `item.id` encoded into the canonical API token; ordinary and broken-target reads use the requested item row, while resolved duplicates use only the authoritative target row.
...
Web-native caveat: on responsive web, edge-swipe means the browser/platform's own history/back gesture where available. It follows the same native Back entry as the browser control: a recorded Feed/Search item origin restores that exact origin, while an external direct item entry follows pre-existing browser history and may leave ResoFeed. It is not the external-entry visible Close action. The sticky row uses the route-aware visible and accessible labels defined in Layout & Spacing. Its external-entry `Return to Feed` / `返回列表` action replaces the current item entry with `/`, so Back after Close cannot reopen the Inspector. If the browser does not expose reliable native edge-swipe, the gesture is a no-op and the sticky back row remains the fallback. Do not implement custom JavaScript physics to imitate iOS/Android edge-swipe. Pull gestures may use pointer/touch handling only for threshold detection and text-state changes; they must not add custom rubber-band animation.
...
- **useFor**: Source rows, OPML source-list import, JSON State export/import, manual `[RUN INGEST]`, per-source `[FETCH]`, and visible current operation status when work is running or blocks an action.
...
- `SOURCE LIST`: `[IMPORT OPML]`. OPML is source intake only: imported feed URLs and feed/source titles are flattened into the Source Ledger. OPML does not export or restore steering rules, stars/resonance, reading history, or any other portable state.
...
Source Ledger bracket command labels are [SHARP] exact English tokens across locales: `[RUN INGEST]`, `[INGESTING...]`, `[FETCH]`, `[FETCHING...]`, `[IMPORT OPML]`, `[IMPORTING OPML...]`, `[EXPORT STATE]`, `[EXPORTING STATE...]`, `[IMPORT STATE]`, `[CONFIRM IMPORT]`, `[IMPORTING STATE...]`, `[DELETE]`, `[CONFIRM DELETE]`, `[DELETING...]`, and `[CANCEL]`. Localize surrounding prose and accessible names, not these visible Source Ledger bracket tokens. UI text must not render lowercase command variants such as `import opml`, `export state`, or `import state`, and must not render localized Source Ledger bracket command labels such as `[导入 OPML]`, `[导出状态]`, or `[导入状态]` unless this contract is explicitly revised. Source row diagnostics are a disclosure, not a command; use low-chrome `source info` / `来源信息`, not `[DETAILS]`.

Canonical Source Ledger surrounding copy by locale is [SHARP]. English: `No sources. Paste RSS URL in Steer.`, `imported N sources; OPML outlines flattened`, `last_ingest: HH:MM:SS local`, `Delete source “<name>”? Future fetches stop; saved items remain.`, `Import State replaces active sources, rules, and stars.`, `OPML = source list; State = sources + rules + stars, import replaces.` zh-CN: `暂无来源。在导向栏粘贴 RSS URL。`, `已导入 N 个来源；OPML 大纲已展平`, `上次摄取: HH:MM:SS 本地`, `删除来源“<name>”？将停止后续抓取；已保存条目保留。`, `导入 State 会替换活动来源、规则和星标。`, `OPML = 来源列表；State = 来源 + 规则 + 星标，导入会替换。` Raw machine diagnostics keep invariant tokens such as `err:`, `op:`, `actor:`, `phase:`, `scope:`, operation kind values, filenames, source titles, URLs, and timestamps.
...
- Mobile/narrow search keeps the single-column route model. Tapping a search result drills into the full-screen Inspector/detail route. Browser/app Back MUST return to the same Search surface with the same query/filter values and re-run lexical retrieval from the canonical URL. The current corpus governs result membership, order, and count, so Back does not promise the same result set. Restore recorded selection and originating-row focus only when both referenced items remain in the current results; if either disappeared, show no selected result and focus the Search query control after retrieval and layout. Restore the prior Search scroll coordinate only after layout and clamp it to the current result-region bounds for the active scroll owner. This restoration is ephemeral navigation state only; it must not create reading history, command history, analytics, or a new product concept.
- Search filter state MUST use the canonical browser URL `/?q={query}[&source=...][&from=...][&to=...][&resonated=true|false][&limit=1..100]` from `docs/ITEM_DEEP_LINKS.md` §7.1. `q` is always present (including `q=`), optional keys appear once in that order, null filters and default `limit=50` are omitted, and canonical UTF-8 query encoding uses `%20` for space and `%2B` for literal plus. Cold load, refresh, Back, and Forward re-run lexical retrieval from that URL; decoded fields must equal HTTP `SearchQueryEcho`, while current corpus changes may alter result membership, order, or count. Selection IDs, scroll coordinates, and focus IDs stay in bounded current-history-entry state; result rows and arrays are never stored in the query string or history. None of this state is a durable saved search, tab system, reading history, or activity ledger.
- Browser acceptance coverage MUST open a filtered Search result, mutate the indexed corpus before Back so the recorded selected/origin item disappears and the current result region becomes shorter, then prove canonical-URL re-retrieval, preserved query/filter echo, no stored result rows/arrays, no restored selection, Search-query focus, and scroll clamped to current result-region bounds.
...
Executing a new desktop Search invalidates any previous TODAY/feed selection as the visible Inspector context. When that deliberate submission returns one or more results, the first result MUST be selected automatically and the desktop Inspector MUST update to that result. This keeps the left Search results and right Inspector in the same information context without requiring an extra click.

History restoration is separate from deliberate Search submission. If its recorded selected item or originating-focus item has disappeared from the re-retrieved current results, the automatic first-result rule does not apply: restore no selection and focus the Search query control.
...
- Route resolution owns the first visible surface and title before token hydration. TODAY, SOURCE LEDGER, SEARCH, direct Inspector routes, Back, and Forward must never expose an intermediate wrong surface or title. Functional document titles remain exact English tokens: `RESOFEED · TODAY`, `RESOFEED · SOURCE LEDGER`, `RESOFEED · SEARCH`, `RESOFEED · INSPECTOR`, and `RESOFEED · /doctor`. Deep-link browser proof must observe the exact Inspector first surface/title for canonical, legacy, and dispatchable invalid item routes before hydration and retain it through authentication recovery; Feed/Search Back and Forward must assert the exact title transition with no intermediate wrong identity.
- The accepted item deep-link delta supersedes the earlier shared-token clause. Direct Inspector browser URLs use `/items/{percent-encoded raw item_id}` under the exact domain in `docs/ITEM_DEEP_LINKS.md`; valid legacy `~base64url` browser routes canonicalize with `replaceState`. Item API requests independently use `/api/items/~{unpadded RFC4648 base64url(item_id)}`. Malformed/non-canonical API tokens do not become item IDs, and UI navigation never derives its browser path from the API token.
- Search URLs use the exact canonical grammar in `docs/ITEM_DEEP_LINKS.md` §7.1: pathname `/`, mandatory `q`, ordered optional `source`, `from`, `to`, `resonated`, and `limit`, canonical UTF-8 query encoding, null/default omission, and field equality with HTTP `SearchQueryEcho`. Valid cold loads, refreshes, Back, and Forward restore fields and re-run lexical retrieval from the URL. Validation occurs before requests or history writes, and history state contains only bounded route primitives—never results, items, saved searches, or reading history.
...
Route resolution precedes token hydration and chooses the correct EN/ZH Today, Source Ledger, Search, Inspector, or `/doctor` surface on cold load, refresh, Back, and Forward. The accepted deep-link delta supersedes this atom's earlier shared-token wording: direct Inspector browser routes use `/items/{percent-encoded raw item_id}` under the exact domain in `docs/ITEM_DEEP_LINKS.md`, including its collision-free `!.` / `!..` sentinels for opaque IDs `.` / `..`, while item API requests independently use `/api/items/~{unpadded RFC4648 base64url(item_id)}`. Valid legacy browser tokens canonicalize with `replaceState`; malformed application paths or API tokens do not become item IDs. Search restores supported lexical fields from route state, validates route and submit input before network/history effects, and persists only bounded route primitives rather than results, items, saved searches, reading history, or another product concept.
...
The generated SvelteKit accessibility announcer follows the same rule. The ResoFeed Svelte preprocessor replaces SvelteKit's generated announcer `style` attribute with the existing `.visually-hidden` class before compilation. This transform is restricted to `.svelte-kit/generated/root.svelte`, preserves the `aria-live="assertive"` and `aria-atomic="true"` semantics, and fails when SvelteKit changes the announcer to a different inline-style contract. Active Svelte source and compiled-output regression coverage must reject inline style attributes, DOM style mutation, and recurrence outside `+page.svelte`; CSP headers remain unchanged.
...
- Do keep Source Ledger flat: view source rows, delete, details, OPML import, JSON State export/import, and lightweight manual ingest/fetch only.
- [SHARP] Do group Source Ledger portability actions by meaning: `SOURCE LIST` for `[IMPORT OPML]`, and `PORTABLE STATE` for `[EXPORT STATE]` / `[IMPORT STATE]`.
- [SHARP] Do make the UI distinction explicit: OPML is source intake only; State backs up/restores active sources, active steering rules, and currently resonated/starred items.
...
- [SHARP] Do support native "Swipe-right to go back" on mobile Inspector routes as the browser/platform Back operation: restore a recorded Feed/Search origin, or follow pre-existing browser history for an external direct entry, without arbitrary or custom animations.
...
- Don't present OPML as export or backup/restore; it is source intake only.
- Don't attach State import's destructive replace warning to OPML import.
...
- **Swipe-right to Go Back**: Mobile narrow-viewport Inspector routes MUST use native OS/browser edge-swipe navigation where available as ordinary browser Back. From a recorded Feed/Search origin it restores that exact origin, scroll, selection where available, and focus; from an external direct entry it follows pre-existing browser history and may leave ResoFeed. The external-entry sticky visible Close is separate: it replaces the item entry with `/`, so later Back cannot reopen the Inspector. If native edge-swipe is unavailable, the gesture is a no-op and the sticky back row remains the fallback. Custom JavaScript edge-swipe physics are forbidden.
...
Purpose: `Escape` is a keyboard escape hatch back to ResoFeed's neutral state, not a command system or configurable shortcut layer. It is distinct from visible Close and browser Back/Forward: those controls restore a recorded Feed/Search origin when one exists.
...
- On a mobile/narrow Inspector route, after inner controls decline the key, `Escape` returns directly to `TODAY` and preserves the feed scroll position. It does not traverse or claim parity with the route-aware visible Close or browser Back behavior; a Search-origin Close/Back still restores Search URL, filters, scroll, and focus.
