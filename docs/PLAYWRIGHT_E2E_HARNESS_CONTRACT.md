...
`scripts/vectl-check.mjs` dispatches by the exact `(suite, check_id)` pair. `select` emits one `vectl.check.selection.v1` envelope with fixed identities and digest. Every recognized `run` invokes only that profile's native checks, emits exactly one valid `vectl.check.evidence.v1` envelope with identical non-empty `selected_ids` and `executed_ids`, and exits the adapter process with status 0 after the envelope is written. The envelope retains the native evidence result: green uses `outcome=green` and `exit_code=0`, while expected red or an execution failure uses `outcome=red` and `exit_code=1`. Unknown suites, unknown checks, cross-paired values, invalid actions, and private build-identity injection remain pre-envelope refusals: they emit no evidence envelope and exit nonzero. Runtime-only secrets remain absent from child environments and evidence.
...
`node scripts/vectl-check.mjs run rf-bug-v2-generic-adapter rf_bug_v2_generic_adapter_green` runs the Node regression and immutable `TestPlaywrightFixtureContract`. The regression validates all twenty-one selection envelopes, parses green/red evidence fixtures, verifies identity parity, accepts only relative-path/SHA-256 artifacts, preserves the completed foundation profile and literal `artifacts: artifactRows` contract, rejects mismatched pairs and malformed artifacts, and confirms protected acceptance scope remains clean. Its focused child-process case also proves that known green and controlled expected-red executions both exit process 0 after exactly one envelope, preserve their `green:0` and `red:1` payload results, retain the item deep-link identities and fingerprints, and keep pre-envelope refusal nonzero without evidence. The item deep-link expected-red native profile snapshots and exactly restores `web/build`, embedded `internal/resofeed/webui`, and stage residue around its Go, Vitest, and Playwright commands so expected-red collection cannot leave generated-path drift. The focused child-process case does not execute unfinished product semantics.
...
- a moving image reference, optional Compose image fallback, incomplete OCI index/platform/revision chain, or platform commit-label verification that does not parse exactly one duplicate-free Buildx JSON object and require the exact revision member to be a string equal to the independently supplied OCI verified commit;
...
- missing strict independent 40-lowercase-hex parsing for `--verified-commit` and `--procedure-source-commit`, inferred/equality-bound source identities, duplicate/missing values, or acceptance of `--no-rollback` in staging, recovery, or orphan modes;
- default deployment without a recoverable prior digest, default rollback drift, or readiness restoration against anything other than the same named volume;
- an explicit forward-only no-rollback path that admits absent, malformed, duplicate, wrong-type, ambiguous, or drifted Serve JSON; validates the route after a persistent effect; derives a prior digest; invokes `rollback_previous_digest`, `ensure_tailscale_serve`, or `tailscale serve`; retries; uses project-wide/build/recreate/restart/Caddy/dependency Compose argv; removes a volume; or omits separate procedure-source and OCI application-source identities;
- absent `success`, `no_effect`, `known_partial`, or `unknown_partial` classification at any exit boundary;
- registry deletion without separate authorization, missing complete orphan-chain recording, secret-value output, credential mutation, owner-token rotation, data clearing, or alternate-target switches;
- drift between the deployment script, Compose/example, Tailnet skill, deployment README, Container guide, and this harness contract.

The focused acceptance test performs procedure staging/recovery and deployment only against temporary local fake SSH, Docker, Tailscale, and readiness surfaces. It verifies the uniform exact SSH option vector; strict existing host-key trust for the literal Tailnet FQDN; unknown and changed key rejection before target-local writes; internal short-hostname irrelevance; canonical path/stack/file/mode checks; exact clean-source binding; attached `HEAD` refusal before any SSH attempt; target-local backups; installed byte/mode identity; dirty-source refusal before SSH; and prior-byte restoration after an injected second-file rename failure.

Its deployment matrix proves spaced-valid semantic Buildx label JSON and rejects malformed JSON, non-object roots, wrong-type revisions, missing revisions, wrong values, and duplicate members without emitting inspected JSON or label values. It also proves default prior-digest success and readiness-checked recovery; default refusal when no prior digest exists; explicit `--no-rollback` success without prior-image inspection; absent, malformed, duplicate, and drifted canonical-route refusal before `.env` or Compose effect; exact resofeed-only config/pull/up argv with `--no-build --no-deps`; no Serve or Caddy effect; missing, duplicate, malformed, and incompatible option refusal; equal and unequal source values without semantic conflation; known-partial and unknown-partial classification; one attempt; named-volume preservation; nondisclosure; and zero external effects. Negative source mutations must fail the adapter.

The profile emits one green evidence envelope only after the focused acceptance test passes. Its observations retain repository/index/platform binding, Tailnet target, moving-tag refusal, default `ROLLBACK=prior_digest_and_readiness`, `NO_ROLLBACK=explicit_forward_only`, `PROCEDURE_SOURCE_COMMIT=required_distinct_from_oci_source`, `OCI_APPLICATION_SOURCE=verified_commit`, `SQLITE_VOLUME=preserved`, masked-presence boundary markers, literal-FQDN strict-known-key endpoint identity, clean-commit two-file staging, `PROCEDURE_SOURCE_CHECKOUT=clean_detached_head`, `PROCEDURE_ATTACHED_HEAD=pre_ssh_rejected`, `PROCEDURE_DETACHED_MATRIX=green`, `PROCEDURE_TAILSCALE_ROUTE_JSON=verified`, `PROCEDURE_TAILSCALE_SERVE_MUTATION=canonical_noop`, prior-byte recovery, classification coverage, and `PROCEDURE_SIDE_EFFECTS=none`. It performs no registry, SSH, Tailnet, Docker, credential, runtime, volume, data, owner-token, route, Caddy, or external mutation.
...
Repository rollback for this deployment-procedure remediation reverts only `deploy/resofeed-caddy/deploy.sh`, `scripts/vectl-check.mjs`, `scripts/vectl-check.test.mjs`, the Tailnet skill, deployment README, Container guide, and this harness contract together. It performs no SSH, procedure recovery, deployment, credential, host-key, Docker/registry, data, volume, owner-token, route, Caddy, or external mutation.
### Tailscale 1.98.8 JSON Route Harness Remediation
The immutable-deployment profile retains the exact selection identity `RF-BUG-V2 immutable OCI and Tailnet deployment procedure`. Its deterministic fake surfaces require `tailscale serve status --json` and accept only a duplicate-free JSON object whose string `TCP.443.TCPForward` equals `127.0.0.1:8443`. Malformed JSON, missing or wrong-type members, duplicate members, extra TCP/443 ownership fields, another target, the obsolete exact row, and human tree output fail closed.

Explicit `--no-rollback` requires the duplicate-free canonical Serve JSON route before every persistent deployment effect. Rejection leaves `.env`, Compose, container, route, Caddy, volume, data, owner-token, credentials, and external state unchanged with `RESULT_CLASSIFICATION=no_effect`. Canonical routing permits one resofeed-only `--no-build --no-deps` Compose update. The exact argv target `resofeed` for config, pull, and up; excludes project-wide reconciliation, Caddy, dependency startup, build, force recreation, and restart; and invokes neither `ensure_tailscale_serve` nor `tailscale serve`.

The passing matrix also preserves default prior-digest rollback, procedure-byte recovery, separate procedure/OCI source identities, image/target/named-volume identities, classification boundaries, nondisclosure, protected state, and no retry. It emits `NO_ROLLBACK_ROUTE=preexisting_canonical_only`, `NO_ROLLBACK_ROUTE_REPAIR=forbidden`, `NO_ROLLBACK_COMPOSE_SCOPE=resofeed_only_no_build`, and `NO_ROLLBACK_CADDY=untouched` only after deterministic local fake coverage passes.

A separate current explicit human authorization may use exactly:

```bash
tailscale serve --yes --bg --tcp=443 tcp://127.0.0.1:8443
```

The executable release chain never contains or invokes that repair. Repository rollback reverts only the admitted seven-file repository change set and performs no SSH, Serve repair, recovery, publication, deployment, or runtime mutation.
