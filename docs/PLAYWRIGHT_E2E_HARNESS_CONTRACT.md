...
- missing detached `HEAD` refusal before any SSH attempt, dirty or mismatched clean-commit two-file procedure staging, wrong Git modes, or a path outside the exact `deploy.sh` and `compose.yml` source pair;
