...
- an explicit forward-only no-rollback path that derives a prior digest, invokes `rollback_previous_digest`, retries deployment, removes/recreates a volume, changes route/Caddy ownership, or omits separate procedure-source and OCI application-source identities;
...
Its deployment matrix proves default prior-digest success and readiness-checked recovery; default refusal when no prior digest exists; explicit `--no-rollback` success without prior-image inspection; missing, duplicate, malformed, and incompatible option refusal before effect; equal and unequal source values accepted without semantic conflation; known-partial and unknown-partial forward-only failure classification; no invocation of old-image recovery; one deployment attempt; named-volume preservation; masked secret nondisclosure; and absence of Docker down/volume deletion, route repair, Caddy reconciliation, registry, credential, owner-token, data, SSH, or external effects. Negative source mutations must fail the adapter.
