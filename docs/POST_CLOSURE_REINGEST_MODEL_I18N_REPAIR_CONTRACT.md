...
- Prompt/model are never persisted to SQLite runtime metadata, state export/import, localStorage, provider config, source/item durable preference state, or command/activity history.
- Prompt/model participate in the idempotency request fingerprint after normalization so same key + same normalized fields replays and same key + different normalized fields returns bad request.
...
Rationale: the bug family asks for one-time extra prompt/model input without durable prompt/model state. Compatibility keeps older clients using `extra_prompt` safe while preserving `prompt` as the canonical field already declared by frontend/types.

Fails if: prompt/model become settings/preferences, extra prompt is silently ignored, `language` is accepted, idempotency ignores prompt/model differences, or error handling leaks provider secrets.
