...
func TestHTTPOPMLExportRoutesRetiredAuthFirst(t *testing.T) {
	router := NewRouter(HTTPServerConfig{OwnerToken: contractOwnerToken})

	for _, path := range []string{"/api/sources/export-opml", "/api/opml/export"} {
		t.Run(path, func(t *testing.T) {
			...
			router.ServeHTTP(authorized, authorizedRequest(http.MethodGet, path, nil))
			assertStatus(t, authorized, http.StatusNotFound)
		})
	}
}
