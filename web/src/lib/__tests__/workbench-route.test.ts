...
type WorkbenchRouteModule = {
  ...
  searchResultActivationLabel(title: string, language: 'en' | 'zh'): string;
  resolveWorkbenchRoute(pathname: string): { itemId: string | null; itemRouteKind: string | null };
};
...
async function loadWorkbenchRoute(): Promise<WorkbenchRouteModule> {
  ...
  try {
    ...
    expect(error, 'expected item route codec module to be importable').toBeUndefined();
    throw error;
  }
}
...
describe('ITEM-DEEP-LINK opaque item IDs', () => {
  it('keeps Search result activation aligned with the Inspector entry action', async () => {
    const route = await loadWorkbenchRoute();
    expect(route.searchResultActivationLabel('Deep link browser primary', 'en')).toBe('Open Inspector for: Deep link browser primary');
    expect(route.searchResultActivationLabel('深度链接', 'zh')).toBe('打开检查器：深度链接');
  });

  it('separates readable application paths from ~base64url API paths', async () => {
    const itemIDs = [
      'item/segment',
      'item%percent',
      '项目/百分号%',
    ...
    for (const itemID of itemIDs) {
      ...
      expect(token, `expected API ~base64url token for ${JSON.stringify(itemID)}`).toBe(expected);
      expect(route.itemAPIPath(itemID)).toBe(`/api/items/${expected}`);
      expect(token).not.toMatch(/=|\+|\//u);
      expect(route.decodeItemRouteToken(token), `expected byte-identical API-token round trip for ${JSON.stringify(itemID)}`).toBe(itemID);
      const appPath = route.itemAppPath(itemID);
      expect(appPath).not.toContain(token);
      expect(route.resolveWorkbenchRoute(appPath)).toMatchObject({ itemId: itemID, itemRouteKind: 'canonical' });
    }
  });
