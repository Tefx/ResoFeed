...
async function waitForDirtyCorpusLifecycle(page: Page): Promise<void> {
  await expect.poll(async () => {
    ...
    return page.evaluate(async () => {
      ...
      const sourcesJson = await sourcesResponse.json() as { sources: Array<{ title: string; last_fetch_status: string; last_fetch_at: string | null }> };
      const feedJson = await feedResponse.json() as { items: Array<{ title: string; source_title: string }> };
      const source = sourcesJson.sources.find((candidate) => candidate.title === 'Dirty Inspector Corpus');
...
test('R1-R8 Inspector browser retest preserves R1 prose while keeping dirty payloads out of primary reading copy', async ({ page, ownerToken, runInfo }, testInfo) => {
  ...
  try {
    ...
    await runSteerCommand(page, 'today', 'today');
    const rowText = await visibleText(page.locator('.contract-feed-item').filter({ hasText: 'Model error keeps raw terse status' }));
    expect(rowText).not.toMatch(/model_status|model_latency_error/i);
    expect(rowText.match(/summary unavailable/gi) ?? []).toHaveLength(1);
    const modelErrorInspector = await openInspectorFor(page, corpusItem('model_error_item'));
